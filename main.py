...
async def dashboard(request: Request):
    ...
    # ===========================================
    # EXCLUIR reclamos SIN lote válido
    # ===========================================
    df["lote"] = df["lote"].astype(str).str.strip().str.lower()
    ...
    # ==================================================
    resumen = df_lotes.groupby(["ean", "lote"]).size().reset_index(name="cantidad_tiendas")

    avisos = resumen[resumen["cantidad_tiendas"] == 2]
    alertas = resumen[resumen["cantidad_tiendas"] >= 3]

    info = df_lotes[["ean", "lote", "descripcion", "proveedor"]].drop_duplicates()
    ...
    filtros = {
        "meses": sorted(df_lotes["mes"].unique()),
        "subtipos": sorted(df_lotes["subtipo"].unique()),
        "calidades": sorted(df_lotes["calidad"].unique()),
        "tiendas": sorted(df_lotes["tienda"].unique())
    }
...
# ======================================================
# ANÁLISIS AVANZADO (TODOS LOS RECLAMOS)
# ======================================================
...
async def analisis(request: Request):
    ...
    # ==========================================
    # Análisis AVANZADO → NO excluye lotes
    # ==========================================
